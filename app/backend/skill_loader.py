...
class SkillLoader:
    ...
    def uninstall_skill(self, name: str) -> Result:
        """Safely deactivate/uninstall a skill from AI runtime.

        Per user specification, uninstalling a skill does NOT delete the physical
        directory or remove the entry from the loaded list. Instead, it marks the
        skill as DISABLED and flags disable_model_invocation = True so it is 100%
        isolated and never used by AI prompts or trigger matching. The card remains
        visible in the UI list for re-enabling at any time.
        """
        entry = self._skills.get(name)
        if not entry:
            return Result.failure(f"Skill not found: {name}")
        ...
        return Result.success(f"Skill safely uninstalled from AI runtime: {name}")

    def get_skill_detail(self, name: str) -> Result:
        entry = self._skills.get(name)
        if not entry:
            return Result.failure(f"Skill not found: {name}")
        body = getattr(entry, "body", "")
        if not body and entry.path:
            sk_file = os.path.join(entry.path, "SKILL.md")
        ...
        return Result.success({
            ...
            "allowed_tools": getattr(entry, "allowed_tools", []),
            "requires": getattr(entry, "requires", []),
