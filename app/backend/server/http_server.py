...
async def handle_marketplace_catalog(request):
    """GET /api/skills/marketplace — List curated open ecosystem skills/plugins/mcp catalog."""
    from skill_registry import get_marketplace_catalog
    category = request.rel_url.query.get("category", "").strip() or None
    query = request.rel_url.query.get("q", "").strip() or None
    type_filter = request.rel_url.query.get("type", "").strip() or None
    catalog = get_marketplace_catalog(category=category, query=query, type_filter=type_filter)
    return web.json_response({"catalog": catalog, "total": len(catalog)})


async def handle_uninstall_skill(request):
    from skill_loader import get_skill_loader
    name = request.match_info.get("name", "")
    if not name:
        return web.json_response({"error": "name required"}, status=400)
    loader = get_skill_loader()
    res = loader.uninstall_skill(name)
    if res.ok:
        return web.json_response({"ok": True, "uninstalled": name, "status": "disabled"})
    return web.json_response({"ok": False, "error": res.error}, status=400)


async def handle_skill_detail(request):
    from skill_loader import get_skill_loader
    name = request.match_info.get("name", "")
    if not name:
        return web.json_response({"error": "name required"}, status=400)
    loader = get_skill_loader()
    res = loader.get_skill_detail(name)
    if res.ok:
        return web.json_response(res.value)
    return web.json_response({"error": res.error}, status=404)


async def handle_install_skill(request):
    """Install a skill from GitHub or curated marketplace (agentskills.io compatible)."""
    from skill_loader import get_skill_loader
    from skill_registry import install_from_github, sync_lockfile, agentskills_info, install_marketplace_skill
    data = await request.json()
    loader = get_skill_loader()
    if data.get("skill_id"):
        result = install_marketplace_skill(str(data["skill_id"]), loader=loader)
        if result.ok:
            return web.json_response(result.value)
        return web.json_response({"error": result.error}, status=400)
    elif data.get("sync_lockfile"):
        ...
        return web.json_response({"error": "skill_id, source+skillPath or sync_lockfile required"}, status=400)
    if result.ok:
...
async def create_server(router: Router, host: str = "127.0.0.1", port: int = 8765):
    ...
    app.add_routes([
        ...
        web.get('/api/skills/marketplace', handle_marketplace_catalog),
        web.post('/api/skills/{name}/enable', handle_enable_skill),
        web.post('/api/skills/{name}/disable', handle_disable_skill),
        web.post('/api/skills/{name}/uninstall', handle_uninstall_skill),
        web.get('/api/skills/{name}', handle_skill_detail),
