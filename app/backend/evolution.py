...
#: 缺省档位。设为谨慎模式 (prudent 50% intensity)。
...
#: 规则行权重标记格式：`<!-- evo:weight=0.6 -->`，写在规则行末尾。
#: 没有标记 = 权重 1.0（向后兼容所有已落盘、未带标记的历史规则行）。
WEIGHT_MARK_RE = re.compile(r"<!--\s*evo:weight=([0-9]*\.?[0-9]+)\s*-->")

#: 权重下限——再降也不能低于它，避免规则权重归零、彻底失去任何约束力。
MIN_RULE_WEIGHT = 0.15

#: 退役阈值——权重降到此值以下、且仍是 ineffective 时，才生成一条“退役提案”
#: 交给人裁决（而不是反复把同一条已证无用的规则塞回用户队列）。
RETIRE_PROPOSAL_THRESHOLD = 0.3

#: 每回效果回访判 ineffective 时权重的衰减系数：1.0 → 0.6 → 0.36 → 0.216(触发退役)。
DEMOTE_FACTOR = 0.6
...
class EvolutionStore:
    ...
    def hot_signatures(self, min_hits: int, limit: int = 20) -> list[dict]:
        ...
        强信号（user_correction, workflow, preference）1 次即达标。
        """
        rows = self._conn.execute(
            ...
            "HAVING hits >= ? OR (MAX(kind) IN ('user_correction', 'workflow', 'preference') AND hits >= 1) "
    ...
    def proposals_with_verdict(self, verdict: str, limit: int = 200) -> list[Proposal]:
        """返回已判定某效果 verdict（effective/improving/ineffective）的提案。

        ineffective 提案是降权/退役逻辑的入口；effective/improving 不会被打扰。
        """
        rows = self._conn.execute(
            "SELECT * FROM proposals WHERE effect=? "
            "ORDER BY effect_reviewed_at DESC LIMIT ?",
            (str(verdict), int(limit)),
        ).fetchall()
        return [self._row_to_proposal(r) for r in rows]

    def set_proposal_draft(self, proposal_id: str, draft: str) -> None:
        """无条件改写一条提案的 draft（含被接受规则的权重标记）。

        与 update_draft 不同：这里不要求 status='pending'——已接受的规则行在
        降权时也要能改。只作废不删除：从不物理移除任何一行，只更新其权重标记。
        """
        clean = PrivacySanitizer.clean(draft) if PrivacySanitizer else draft
        self._conn.execute(
            "UPDATE proposals SET draft=? WHERE id=?",
            (clean, proposal_id),
        )
        self._conn.commit()
    ...
    * ``decide`` —— 接受或拒绝一条提案。
    ...
    def record_user_correction(self, turn_id: str, correction_text: str, session_id: str = "") -> Optional[str]:
        ...
        return self.record(
            ...
            detail=f"user correction: {correction_text}",
    ...
    def mine(self) -> list[Proposal]:
        ...
        for row in hot:
            ...
            try:
                from evolution_tools import EvolutionDraftManager
                draft_mgr = EvolutionDraftManager(workspace_root=self.workspace_root)
                trig = "correction" if proposal.kind == "user_correction" else "preference"
                draft_mgr.save_pending_draft(proposal, trigger_type=trig)
    ...
    def decide(self, proposal_id: str, accept: bool,
        ...
        if not accept:
            ...
            try:
                from evolution_tools import EvolutionDraftManager
                EvolutionDraftManager(workspace_root=self.workspace_root).mark_rejected(proposal_id)
            except Exception:
                pass
        ...
        if applied:
            try:
                from evolution_tools import EvolutionDraftManager
                EvolutionDraftManager(workspace_root=self.workspace_root).mark_approved(proposal_id)
            except Exception:
                pass
            try:
...
def _rule_weight(line: str) -> float:
    """从一条规则行读出权重。

    没有标记就是 1.0（向后兼容所有已落盘、未带标记的历史规则行）；标记解析失败
    也退回 1.0，绝不让一条坏标记把规则静默升到零权重。
    """
    if not line:
        return 1.0
    m = WEIGHT_MARK_RE.search(line)
    if not m:
        return 1.0
    try:
...
def _set_weight_mark(line: str, weight: float) -> str:
    """把规则行的权重标记设成 weight；没有就追加到末尾，有就原地替换。

    只动末尾的标记，不动规则正文——保证同一行规则的含义不变，只是权重变了。
    """
    base = WEIGHT_MARK_RE.sub("", line).strip()
    return f"{base} <!-- evo:weight={weight} -->"


def _demote_proposal_for(p: Proposal, new_draft: str) -> Proposal:
    """构造一条用于写盘（替换原行）的提案镜像，supersedes 指向旧的未标记行。"""
    return Proposal(
        id=p.id, signature=p.signature, kind=p.kind, tool_name=p.tool_name,
        target_file=p.target_file, draft=new_draft, rationale=p.rationale,
        hits=p.hits, status=p.status, created_at=p.created_at,
        decided_at=p.decided_at, applied=p.applied, supersedes=p.draft,
    )


def _build_retire_proposal(old: Proposal, demoted_draft: str) -> Proposal:
    """为一条已降到退役阈值以下、仍 ineffective 的规则生成退役提案，交人裁决。"""
    return Proposal(
        id=uuid.uuid4().hex[:12],
        signature=old.signature,
        kind="rule_retire",
        tool_name=old.tool_name,
        target_file=old.target_file,
        draft=demoted_draft,
        rationale=f"规则权重已降至退役阈值以下且效果仍无效，提议退役：{old.draft}",
        hits=old.hits,
        status="pending",
        created_at=time.time(),
        supersedes=old.draft,
    )


def demote_ineffective_rules(engine) -> list[dict]:
    """对被判 ineffective 的规则做降权，而不是重新塞回用户队列（纪律 2：宁可不提，不可骚扰）。

    每条 ineffective 规则把权重乘以 DEMOTE_FACTOR（1.0 → 0.6 → 0.36 → …），
    权重不跌破 MIN_RULE_WEIGHT。权重降到 RETIRE_PROPOSAL_THRESHOLD(0.3) 以下、
    且仍是 ineffective 时，才生成一条“退役提案”交给人裁决——只作废不删除，绝不
    在本任务里物理移除任何规则行或数据库行。

    返回每条被降权规则的 {"id", "from", "to", "retire_proposal"}，供调用方知悉
    本次动作；没有任何 ineffective 规则时返回空列表。
    """
    out: list[dict] = []
    for p in engine.store.proposals_with_verdict("ineffective"):
        old_w = _rule_weight(p.draft)
        new_w = round(old_w * DEMOTE_FACTOR, 4)
        if new_w < MIN_RULE_WEIGHT:
            new_w = MIN_RULE_WEIGHT
        new_draft = _set_weight_mark(p.draft, new_w)

        # 1) 落盘权重标记到提案 draft
        engine.store.set_proposal_draft(p.id, new_draft)
        # 2) 把权重标记写进引导文件里那一行（替换而非追加，复用既有写盘路径）
        engine._apply_to_guidance(_demote_proposal_for(p, new_draft))

        # 3) 降到退役阈值以下且仍无效，才生成退役提案交给人裁决
        retire = new_w < RETIRE_PROPOSAL_THRESHOLD
        if retire:
            engine.store.insert_proposal(_build_retire_proposal(p, new_draft))
        out.append({
            "id": p.id,
            "from": old_w,
            "to": new_w,
