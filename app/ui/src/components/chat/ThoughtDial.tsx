// ThoughtDial — Clean White Track with Rhythmic Square Energy Burst at MAX.
//
// At normal levels (off, low, high), it provides a calm, clean, smooth
// light-blue fill along a crisp white rail. When dialed to MAX, it erupts into
// a vibrant rhythmic energy surge filled with sparkling, dancing square quanta.
...
// 100% Densely Packed Honeycomb Square Block Matrix — Hardware Accelerated & Zero-Jank
//
// Stays mounted and pre-warmed. When entering MAX:
// - Slowly, gracefully awakens from the left edge and fills across to the right over 1.1s.
// - Zero GC allocation per frame via pre-baked color LUT.
// - Hardware-accelerated GPU composite layer (transform: translateZ(0)).
function ThoughtMaxBlockMatrix({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    const OPEN_DURATION = 1.1 // 1.1s slow, graceful left-to-right filling flow

    // Measure dimensions once per activation / resize (zero layout thrash in render loop)
    const updateSize = () => {
    ...
    const render = (now: number) => {
      ...
      // Opening wave front from left to right (smooth fluid cubic ease-out)
      const openRatio = Math.min(1, t / OPEN_DURATION)
      const openFront = 1 - Math.pow(1 - openRatio, 2.6)

      // Steady, organic wind speed flowing left to right
      const windSpeed = 3.0
      ...
      for (let c = 0; c < cols; c++) {
        ...
        // If the initial opening wave hasn't reached this column yet
        if (u > openFront) {
          continue
        }

        // Soft entry opacity fade for columns near the leading front
        const distBehind = openFront - u
        const colAlpha = openRatio < 1 ? Math.min(1.0, distBehind / 0.12) : 1.0

        // Multi-frequency wind waves traveling from left to right
      ...
      <div className="relative">
        ...
        >
          ...
          {hasFill && (
            ...
            >
              {/* Full 2D Honeycomb Block Matrix — Pre-warmed & Hardware Accelerated */}
              <ThoughtMaxBlockMatrix active={atMax} />
            </motion.div>
          )}

          {/* Smooth White Capsule Thumb */}
          <motion.div
            aria-hidden
            initial={false}
            animate={{
            ...
            className={cn(
              'thought-dial-thumb',
              atMax && 'thought-dial-thumb--max',
            )}
          >
            {/* Subtle center grip pill */}
            <span className={cn(
              'w-0.5 h-2 rounded-full transition-colors',
              atMax ? 'bg-sky-400' : 'bg-slate-300',
            )} />
          </motion.div>
        </div>
      </div>

      {/* Stop scale labels underneath */}
        ...
        {steps.map((step, i) => {
          ...
          return (
            <button
              key={step.id}
              type="button"
              data-testid="chat-thought-level-select-item"
              onClick={() => setIndex(i)}
              className={cn(
                'text-[10.5px] leading-none py-0.5 transition-colors duration-150 select-none font-medium',
                isActive
