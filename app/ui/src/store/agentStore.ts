...
import { AgentEvent, InlineEvolutionProposal, Message, ReasoningBlock, SubagentActivity, SubagentEntry, SubagentInfo, SubagentToolCall, ToolCall, ToolCallPreview, ToolCallStatus, TurnStatus } from '@apptypes/index'
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === 'agent_delta') {
          ...
          // 在聊天流中就地呈现自进化卡片 (In-Chat Evolution Proposal Card)
          const p = data.payload || {}
          const proposalData: InlineEvolutionProposal = p.proposal || p
          if (proposalData && (proposalData.proposal_id || proposalData.id || proposalData.proposed_change)) {
            const proposalId = proposalData.proposal_id || proposalData.id || `prop-${Date.now()}`
            const exists = get().messages.some(
              m => m.kind === 'evolution_card' &&
              (m.evolutionProposal?.proposal_id === proposalId || m.evolutionProposal?.id === proposalId)
            )
            if (!exists) {
              const cardMsg: Message = {
                id: `evo-card-${proposalId}`,
                role: 'system',
                content: '',
                timestamp: Date.now(),
                kind: 'evolution_card',
                evolutionProposal: proposalData,
              }
              set(state => ({ messages: [...state.messages, cardMsg] }))
            }
          }
