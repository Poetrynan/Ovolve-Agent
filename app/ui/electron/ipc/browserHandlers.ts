...
// Bridges the renderer's Browser panel to the existing BrowserManager HTTP
// bridge on 127.0.0.1:8766. Going through IPC (rather than fetch from the
// renderer) keeps the loopback port off the renderer's network surface.
...
function callBridge(
  ...
  return new Promise((resolve) => {
    ...
    // The bridge authenticates every request; without the token it answers 401
    // (see browserManager.authorize). It is the same secret the Python backend
    // uses, handed to us through the IPC context.
